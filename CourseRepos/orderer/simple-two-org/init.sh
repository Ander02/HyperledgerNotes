...
export BASE_CONFIG_DIR=../../setup/config/simple-two-org
...
if [ ! -z $1 ]; then
    if [ $1 == "all" ]; then
        ...
        cp ../../setup/config/simple-two-org/crypto.1/crypto-config.yaml .
        cp ../../setup/config/simple-two-org/policy.1/configtx.yaml .
        cp ../../setup/config/simple-two-org/orderer.1/orderer.yaml .
